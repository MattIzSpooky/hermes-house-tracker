...
Two Flyway migrations, not one — split because `ChatMessageRepositoryTest` is the only repository test that validates against real Flyway-migrated Postgres (`ddl-auto=validate`); every other encrypted entity's test runs against H2 with Hibernate auto-generating the schema. That means `chat_messages.encryption_key_version` has to physically exist as soon as the entity declares it, before the broader migration would otherwise land:

- `V15__add_chat_messages_encryption_key_version.sql`: adds only `encryption_key_version INT NOT NULL DEFAULT 1` to `chat_messages` — purely additive, no truncate needed.
- `V16__encrypt_sensitive_columns.sql`:
  ...
  - Add `encryption_key_version INT NOT NULL DEFAULT 1` to `notifications`, `user_profiles`, and `agent_tasks` (not `chat_messages` — already added by `V15`).
